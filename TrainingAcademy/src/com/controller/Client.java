					...
					//obj.setsMobileNo(sc.next());
					 ...
					 if(!str.trim().equalsIgnoreCase("na")) {
						 if (isValidMobileNo(str))  {
								...
								System.out.println("Invalid Mobile Plz ReENter no.");      
							}  
						}
...
//					double phone=sc.nextDouble();
		...
		public void top5StudentOfBatch(String batch_Name) {
			 ...
			 * 
			 * select * from 
      (select S.S_Id,B.Batch_Name,S.S_Name,S.Batch_No,E.E_percentage from student S inner join exam_result E on S.S_Id=E.E_Id 
            inner join Batch B on E.Batch_No=B.Batch_NO ) A where Batch_Name="Csharp" order by E.E_percentage ;
			 * 
			 * */
			
			
			RowSet rs=db.getAll("select * from (select S.S_Id,B.Batch_Name,S.S_Name,S.Batch_No,E.E_percentage from student S inner join exam_result E on S.S_Id=E.E_Id inner join Batch B on E.Batch_No=B.Batch_NO ) A where Batch_Name=? order by E.E_percentage ",batch_Name);
		...
		public void top10Students() {
			RowSet rs=db.getAll("select student.S_Id,student.S_Name,exam_result.E_Percentage from exam_result,student where student.S_Id=exam_result.S_Id order by exam_result.E_Percentage desc limit 5;");
		...
		public void topAvaragePercentage() {
			...
			RowSet rs=db.getAll("select T_Name,Batch_Name from trainer,batch where trainer.Batch_No=batch.Batch_NO and batch.Batch_No in (select tmp1.Batch_No from (select max(tmp.ap),Batch_No from (select Batch_No,avg(E_Percentage) as ap from exam_result group by Batch_no) tmp) tmp1)");
